...
const ContactCard = ({imageUrl, description, name, linkedIn, website}) => {
    ...
    return(
        <div className="flex bg-[#ffffff55] backdrop-blur box-border border-white border-[2px] p-4 shadow-md mt-10 w-[80vw] max-w-[800px] rounded-md">
            ...
            <a href={linkedIn} className="py-2 px-4 rounded-md transition-transform transform-gpu hover:scale-105 m-5 w-[120px]" target="blank"><img src="src/assets/Linkedin.png" alt="LinkedIn link" /></a>
            <a href={website} className="bg-violet-700 text-white py-2 px-4 rounded-md hover:bg-violet-950 transition-transform transform-gpu hover:scale-105 m-5" target="blank">Website</a>
            </div>
        </div>
    )
}
